...
static void report_endpoint_forces(
    ...
    const double* s1, double c, const double* g_vec)
{
    double F_P1[3] = {0}, F_P2[3] = {0};

    // Force on P1 from spring to x[0]
    compute_spring_force(P1, &x[0], s1[0], c, F_P1);

    // Force on P2 from spring to x[n-2]
    compute_spring_force(P2, &x[(n - 2) * 3], s1[n - 1], c, F_P2);
    ...
    double d_dot_g = d_unit[0]*g_unit[0] + d_unit[1]*g_unit[1] + d_unit[2]*g_unit[2];
    double e_plane[3] = {
    ...
    // === NEW: Net force sum over all masses ===
     ...
     int n, double c, double m, const double* g_vec, double scale_pos, double* s0_relaxed)
{
    ...
    if (angle_deg < 10.0 || angle_deg > 170.0) {
        ...
        // Compute segment lengths and store in s0_relaxed
        double s0_sum = 0;
        for (int i = 0; i < n; ++i) {
            double xi[3], xi1[3];
    ...
    double dt_given, int max_steps, double scale_pos)
{
    ...
    double dt = fmin(dt_given, 0.01 / omega); // smaller safe starting dt
    double damping = 1.5 * sqrt(c * m);       // stronger overdamping
...
/*
        ...
        // === Adaptive damping and timestep ===
        if (step % 100 == 0) {
            if (damping > damping_target)
                damping *= 0.90; // faster decay
            if (v_norm_sq < prev_v_norm && dt < 0.1 / omega)
                dt *= 1.02;
            prev_v_norm = v_norm_sq;
        }

        // === Velocity reset if nearly converged ===
        if (v_norm_sq < 1e-10 && step > 1000) {
            for (int i = 0; i < dof; ++i)
                v[i] = 0.0;
        }

        // === Force stillness on plateau ===
    ...
    // Recompute spring lengths after relaxation
    for (int i = 0; i < n; ++i) {
        double xi[3], xi1[3];
...
{ 
    // Scaling conventions:
    // - Positions:        x_scaled = x / L
    // - Gravity:          real g_vec unscaled 
    // - Mass:             real mass used
    // - Stiffness:        real c used
    ...
    if (!x || !res || !J || s0_relaxed || s1 || dx) return -1;
...
*/
    for (int iter = 0; iter < MAX_ITER; ++iter) {
        compute_residuals(x, P1_scaled, P2_scaled, n, s0_relaxed  , c_scaled, m_scaled, g_vec_scaled, scale_pos, res);
        double res_norm = 0.0;
        ...
        for (int i = 0; i < dof; ++i) {
            ...
            compute_residuals(x, P1_scaled, P2_scaled, n, s0_relaxed, c_scaled, m_scaled, g_vec_scaled, scale_pos, res_plus);
            x[i] = orig - eps;
            double *res_minus = malloc(dof * sizeof(double));
            compute_residuals(x, P1_scaled, P2_scaled, n, s0_relaxed, c_scaled, m_scaled, g_vec_scaled, scale_pos, res_minus);
            x[i] = orig;
            for (int j = 0; j < dof; ++j)
        ...
        for (int trial = 0; trial < LS_MAX_TRIALS; ++trial) {
            ...
            compute_residuals(x_trial, P1_scaled, P2_scaled, n, s0_relaxed, c_scaled, m_scaled, g_vec_scaled, scale_pos, res_trial);
    ...
    report_endpoint_forces(P1, P2, x, n, s1, c, g_vec);
