    ...
    double dt_given, int max_steps)
{
    ...
    // --- NEW: Initialize x[] with sagging shape ---
    
    #define USE_CATENARY_INIT 1  // Toggle between 1 (catenary) and 0 (parabola)
    ...
    // Gravity magnitude
    double g_norm = sqrt(g_vec[0]*g_vec[0] + g_vec[1]*g_vec[1] + g_vec[2]*g_vec[2]);
    double T0 = c * s0; // estimate small initial tension
    double a = (T0 > 0 && g_norm > 0) ? (T0 / (m * g_norm)) : 1.0; // fallback safe
    ...
    #if USE_CATENARY_INIT
        // === Catenary sag initialization ===
        double s = t * L;
        double sag_amount = a * (cosh(s / a) - 1.0); // vertical sag
    #else
        // === Parabolic sag initialization ===
        double sag_amount = 0.5 * g_norm * t * (1.0 - t);
    #endif

        // Apply sag in gravity direction
        for (int j = 0; j < 3; ++j)
            x[i*3+j] += sag_amount * (g_vec[j] / g_norm);
    }
    ...
    for (int step = 0; step < max_steps; ++step) {
        ...
        // ==== NEW: Add penalty force to keep near the rope line ====

        double p_minus_p0[3] = {mid[0] - P1[0], mid[1] - P1[1], mid[2] - P1[2]};
        double t_proj = (p_minus_p0[0]*dx_line[0] + p_minus_p0[1]*dx_line[1] + p_minus_p0[2]*dx_line[2]) / (L*L);
        double projection[3] = {
            P1[0] + t_proj * dx_line[0],
            P1[1] + t_proj * dx_line[1],
            P1[2] + t_proj * dx_line[2]
        };
        double deviation[3] = {
            mid[0] - projection[0],
            mid[1] - projection[1],
        ...
        double t_lateral = (deviation[0]*dx_line[0] + deviation[1]*dx_line[1] + deviation[2]*dx_line[2]) / (L*L);
        double deviation_along[3] = {
            t_lateral * dx_line[0],
            t_lateral * dx_line[1],
            t_lateral * dx_line[2]
        };
        ...
        double k_lateral_correction = 0.2 * c;  // or maybe 0.1 * c if needed
    
        for (int j = 0; j < 3; ++j)
            F[i*3+j] += -k_lateral_correction * deviation_lateral[j];
    }
