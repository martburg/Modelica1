    ...
    double step_size, int max_iter, double tol)
{
    ...
    if (!grad || !grad_prev || !s || !y || !H || !p || !x_prev || !Hy || !x_trial ){
        free(grad); free(grad_prev); free(s); free(y); free(H); free(p); free(x_prev); free(Hy);free(x_trial);
        return -1;}
    ...
    for (int iter = 0; iter < max_iter; ++iter) {
        ...
        for (int i = 0; i < dof; ++i) {
            if (!isfinite(grad[i])) {
                log_error("NaN in gradient at iter %d, index %d\n", iter, i);
        ...
        for (int i = 0; i < dof; ++i) {
            ...
            if (!isfinite(x_trial[i])) {
                log_error("NaN in x_trial at iter %d, index %d\n", iter, i);
                free(grad); free(grad_prev); free(s); free(y); free(H);
                free(p); free(x_prev); free(Hy);free(x_trial);
                return SOLVE_ERROR_NAN;
            }
        }
        ...
        if (!isfinite(energy)) {
            log_error("NaN in energy at iter %d\n", iter);
            free(grad); free(grad_prev); free(s); free(y); free(H);
            free(p); free(x_prev); free(Hy);free(x_trial);
            return SOLVE_ERROR_NAN;
        }
        for (int i = 0; i < dof; ++i) {
            if (!isfinite(grad_trial[i])) {
                log_error("NaN in trial gradient at iter %d, index %d\n", iter, i);
                free(x_trial);
                free(grad); free(grad_prev); free(s); free(y); free(H);
                free(p); free(x_prev); free(Hy);
                return SOLVE_ERROR_NAN;
        ...
        if (fabs(energy - prev_energy) < tol * 1e-2 || energy < prev_energy) {
            ...
            log_error("Rejected step at iter %d: energy increased\n", iter);
            free(grad); free(grad_prev); free(s); free(y); free(H);
            free(p); free(x_prev); free(Hy);free(grad_trial);free(x_trial);
        ...
        for (int i = 0; i < dof; ++i) {
            if (!isfinite(s[i]) || !isfinite(y[i])) {
                log_error("NaN in s or y at iter %d, index %d\n", iter, i);
                free(grad); free(grad_prev); free(s); free(y); free(H);
                free(p); free(x_prev); free(Hy);free(x_trial);
                return SOLVE_ERROR_NAN;
            }
