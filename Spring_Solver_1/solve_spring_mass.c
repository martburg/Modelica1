    ...
    double* out_positions)
{
    ...
    double preload_force = 0.5 * m * g_norm;  // guess: spring holds half its own weight
    double l0 = L / n;
    double l = l0 + preload_force / c;
    double d = l0;  // assume horizontal projection still ~ l0
    ...
    for (int i = 0; i < n-1; ++i) {
        ...
        for (int j = 0; j < 3; ++j)
            x[i*3 + j] = r_linear[j] + sag_profile * g_vec_scaled[j];
        //printf("Init[%d]: [%f, %f, %f]\n", i, x[i*3+0], x[i*3+1], x[i*3+2]);
    }

    //printf("Starting dynamic relaxation...\n");
    dynamic_relaxation(x, P1_scaled, P2_scaled, n, s0_scaled, c_scaled, m_scaled, g_vec_scaled, 0.01, 1000);
    //printf("Dynamic relaxation complete.\n");

    //printf("Post-Relax positions:\n");
    //for (int i = 0; i < n-1; ++i) {
    //    printf("Relaxed[%d]: [%f, %f, %f]\n", i, x[i*3+0], x[i*3+1], x[i*3+2]);
    //}
