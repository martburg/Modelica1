    ...
    double dt_given, int max_steps, double scale_pos)
{
    ...
    double damping_target = 1.2 * sqrt(c / m);
    ...
    double* out_positions)
{
    ...
    // - Gravity:          real g_vec scaled with m * g_borm
    // - Mass:             real mass  scaled with m * g_norm
    // - Stiffness:        real c     scaled with m * g_normd
...
*/
    ...
    free(x); free(res); free(J);free(s0_relaxed);free(dx);
    return 0;
}
