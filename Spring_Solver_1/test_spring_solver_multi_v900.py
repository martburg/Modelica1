...
def run_and_test(P1, P2, n, total_mass, length_factor, rope_diameter,
                 youngs_modulus, g_vec, tol_percent=2.5, verbose=True):
    ...
    res = lib.solve_spring_mass_c(
        ...
        ctypes.byref(Length_Newton),
        ctypes.byref(Status_Dynamic), ctypes.byref(Status_Newton)
    )
    print(Status_Dynamic.value)
    print(Status_Newton.value)

    # Check solver statuses
    if Status_Dynamic.value != 0 or Status_Newton.value != 0 :
        if verbose:
            print("❌ Solver status failed.")
            print(f"Status_Dynamic = {Status_Dynamic.value}, Status_Newton = {Status_Newton.value}")
        return False
