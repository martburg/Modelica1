    ...
    double *grad)
{
    ...
    for (int i = 0; i < n; ++i) {
        const double *xi = (i == 0) ? P1 : &x[(i - 1) * 3];
        const double *xj = (i == n - 1) ? P2 : &x[i * 3];

        double d[3] = { xj[0] - xi[0], xj[1] - xi[1], xj[2] - xi[2] };
        double L = norm3(d);
        if (L < 1e-8) L = 1e-8;
        ...
        for (int j = 0; j < 3; ++j) {
            double f = coeff * d[j];
            if (i > 0) grad[(i - 1) * 3 + j] -= f;
            if (i < n - 1) grad[i * 3 + j] += f;
        }
    }
    for (int i = 0; i < n - 1; ++i) {
...
static double wolfe_line_search(
    const double *x, const double *p, int dof,
    const double *P1, const double *P2, int n,
    const double *s0, double k, double m, const double *g_vec,
    double *grad, double *new_x, double *new_grad,
    double c1, double c2, double alpha_init, double alpha_max)
{
    double alpha = alpha_init;

    double phi0 = compute_total_energy(x, P1, P2, n, s0, k, m, g_vec);
    double dphi0 = 0.0;
    for (int i = 0; i < dof; ++i)
        dphi0 += grad[i] * p[i];
    ...
    for (int iter = 0; iter < 20; ++iter) {
        ...
        compute_energy_gradient(new_x, P1, P2, n, s0, k, m, g_vec, new_grad);
        double phi = compute_total_energy(new_x, P1, P2, n, s0, k, m, g_vec);

        double dphi = 0.0;
        for (int i = 0; i < dof; ++i)
            dphi += new_grad[i] * p[i];

        if (phi > phi0 + c1 * alpha * dphi0 || (iter > 0 && phi >= phi0)) {
            alpha *= 0.5;
        } else if (fabs(dphi) <= -c2 * dphi0) {
            return alpha;
        } else if (dphi >= 0.0) {
            alpha *= 0.5;
        } else {
            alpha = fmin(alpha * 2.0, alpha_max);
        }
    }

    return alpha_init * 0.1;  // fallback
}
    ...
    double step_size, int max_iter, double tol)
{
    ...
    for (int iter = 0; iter < max_iter; ++iter) {
        ...
        double alpha = wolfe_line_search(
            x_out, p, dof, P1, P2, n, s0, k, m, g_vec,
            grad, x_temp, grad_temp,
            1e-4, 0.9, step_size, 10.0);
