    ...
    double* out_positions)
{
    ...
    if (cos_theta > 0.9) {
        printf("[Init] Cable is nearly vertical (cos(theta) = %.6f). Pre-stretched vertical initialization.\n", cos_theta);

        // Pre-stretch nodes according to gravity load
        double unstretched_length = L / n;
        double total_vertical = P2_scaled[2] - P1_scaled[2];
        double z_curr = 0.0;
        for (int i = 0; i < n-1; ++i) {
            double weight_below = (n - i - 0.5) * m * g_norm;
            double stretch = weight_below / c;
            double segment_length = unstretched_length + stretch;
            z_curr += segment_length;

            // Proportion along the full length
            double proportion = z_curr / (1.0 + (total_mass * g_norm) / (c * L));  // Corrected total stretch
            for (int j = 0; j < 3; ++j)
                x[i*3 + j] = (1.0 - proportion) * P1_scaled[j] + proportion * P2_scaled[j];
        }
    } else {
