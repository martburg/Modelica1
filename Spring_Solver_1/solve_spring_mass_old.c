    ...
    double s0, double c, double m, double* g_vec, double *residuals_out)
{
    ...
    // Soft spring stiffness: 10% of main spring stiffness (tunable)
    double k_soft = 0.1 * c;

    // Precompute P1->P2 vector
    ...
    for (int i = 0; i < n - 1; ++i) {
        ...
        // Soft spring penalty (pull toward rope line)
        ...
        for (int j = 0; j < 3; ++j) {
            double deviation = mid[j] - projection[j];
            residuals_out[i * 3 + j] += -k_soft * deviation;
        }
    ...
    double* out_positions)
{
    ...
    for (int iter = 0; iter < MAX_ITER; ++iter) {
        ...
        for (int trial = 0; trial < LS_MAX_TRIALS; ++trial) {
            ...
            compute_residuals(x_trial, P1_scaled, P2_scaled, n, s0_scaled, c_scaled, m_scaled, g_vec_scaled, res_trial);
