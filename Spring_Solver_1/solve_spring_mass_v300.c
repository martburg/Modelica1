...
int newton_solver_analytic_2d(
    double *x,               // internal node positions: 2*(n-1)
    const double *P1,        // endpoint 1 [2]
    const double *P2,        // endpoint 2 [2]
    int n,                   // number of nodes (including endpoints)
    const double *s0,        // segment rest lengths [n]
    double k,                // spring stiffness
    double m,                // mass per node
    const double *g_vec      // gravity vector [2]
) {
    int dof = 2 * (n - 1);
    double *res = calloc(dof, sizeof(double));
    double *J   = calloc(dof * dof, sizeof(double));
    int *ipiv   = calloc(dof, sizeof(int));
    if (!res || !J || !ipiv) return -1;
    ...
    for (int iter = 0; iter < MAX_ITER; ++iter) {
        ...
        for (int i = 0; i < n - 1; ++i) {
            ...
            const double *x_prev = (i == 0)     ? P1 : &x[(i - 1) * 2];
            const double *x_next = (i == n - 2) ? P2 : &x[(i + 1) * 2];

            double F_prev[2], F_next[2];
            compute_spring_force_2d(x_prev, xi,     s0[i],     k, F_prev);
            compute_spring_force_2d(x_next, xi,     s0[i + 1], k, F_next);

            res[i * 2 + 0] = F_prev[0] + F_next[0] + m * g_vec[0];
            res[i * 2 + 1] = F_prev[1] + F_next[1] + m * g_vec[1];

            // Diagonal blocks: ∂F/∂xi
            add_spring_jacobian_2d(J, i, i,     dof, xi, x_prev, s0[i],     k, +1);
            add_spring_jacobian_2d(J, i, i,     dof, xi, x_next, s0[i + 1], k, +1);

            // Off-diagonal blocks: ∂F/∂xj
            if (i > 0)
                add_spring_jacobian_2d(J, i, i - 1, dof, xi, x_prev, s0[i],     k, -1);
            if (i < n - 2)
                add_spring_jacobian_2d(J, i, i + 1, dof, xi, x_next, s0[i + 1], k, -1);
        }

        // Solve J dx = -res using LAPACK
        for (int i = 0; i < dof; ++i)
            res[i] *= -1;

        int info, nrhs = 1, lda = dof, ldb = dof;
        dgesv_(&dof, &nrhs, J, &lda, ipiv, res, &ldb, &info);
        if (info != 0) {
            printf("❌ dgesv failed (info = %d)\n", info);
            free(res); free(J); free(ipiv);
            return -1;
        }
        ...
        // printf("Newton iter %d: step norm = %.3e\n", iter, norm_dx);

        if (norm_dx < TOL) {
            printf("\nNewton 2D CONVERGED in %d steps.\n", iter);
            break;
        }
    }
...
int newton_solver_2d(
    const double *P1_3D, const double *P2_3D,
    const double *g_3D,
    const double *x_3D_in,  // input: initial guess (3D, length 3*(n-1))
    int n,
    const double *s0,       // rest lengths (n segments)
    double k,               // stiffness
    double m,               // mass per node
    double *x_3D_out        // output: final 3D positions (3*(n-1))
) {
 
    const int dof_2D = 2 * (n - 1);

    // Step 1: compute rope–gravity plane
    double R_3x2[6];
    compute_rope_plane_basis(P1_3D, P2_3D, g_3D, R_3x2);

    // Step 2: project endpoints and gravity into 2D
    double P1_2D[2], P2_2D[2], g_2D[2];
    for (int i = 0; i < 2; ++i) {
        P1_2D[i] = P1_3D[0] * R_3x2[i] + P1_3D[1] * R_3x2[i + 2] + P1_3D[2] * R_3x2[i + 4];
        P2_2D[i] = P2_3D[0] * R_3x2[i] + P2_3D[1] * R_3x2[i + 2] + P2_3D[2] * R_3x2[i + 4];
        g_2D[i]  = g_3D[0]  * R_3x2[i] + g_3D[1]  * R_3x2[i + 2] + g_3D[2]  * R_3x2[i + 4];
    }

    // Step 3: project initial guess into 2D
    double *x_2D = malloc(dof_2D * sizeof(double));
    if (!x_2D) return -1;
    project_to_2d_rope_frame(x_3D_in, n, R_3x2, x_2D);

    double *s0_2D = malloc(n * sizeof(double));
for (int i = 0; i < n; ++i) {
    double p1[3], p2[3];

    if (i == 0) {
        memcpy(p1, P1_3D, 3 * sizeof(double));
        memcpy(p2, &x_3D_in[0], 3 * sizeof(double));
    } else if (i == n - 1) {
        memcpy(p1, &x_3D_in[3 * (n - 2)], 3 * sizeof(double));
        memcpy(p2, P2_3D, 3 * sizeof(double));
    } else {
        memcpy(p1, &x_3D_in[3 * (i - 1)], 3 * sizeof(double));
        memcpy(p2, &x_3D_in[3 * i], 3 * sizeof(double));
    }
    ...
    // Project into rope plane
    double d2[2] = {
        d3[0]*R_3x2[0] + d3[1]*R_3x2[2] + d3[2]*R_3x2[4],
        d3[0]*R_3x2[1] + d3[1]*R_3x2[3] + d3[2]*R_3x2[5]
    };

    s0_2D[i] = sqrt(d2[0]*d2[0] + d2[1]*d2[1]);
}

    // Step 4: run 2D Newton solver
    int status = newton_solver_analytic_2d(x_2D, P1_2D, P2_2D, n, s0_2D, k, m, g_2D);
    if (status != 0) {
        printf("❌ 2D Newton solver failed\n");
        free(x_2D);
        return status;
    }

    // Step 5: reconstruct final 3D positions
    reconstruct_from_2d_rope_frame(x_2D, n, R_3x2, x_3D_out);
    free(x_2D);
    return 0;
}
...
int analytic_newton_solver_3d(
    double *x,               // internal node positions: 3*(n-1)
    const double *P1,        // endpoint 1 [3]
    const double *P2,        // endpoint 2 [3]
    int n,                   // number of nodes (including endpoints)
    const double *s0,        // segment rest lengths [n]
    double k,                // spring stiffness
    double m,                // mass per node
    const double *g_vec      // gravity vector [3]
) {
    int dof = 3 * (n - 1);
    double *res = calloc(dof, sizeof(double));
    double *J   = calloc(dof * dof, sizeof(double));
    ...
    for (int iter = 0; iter < MAX_ITER; ++iter) {
        ...
        for (int i = 0; i < n - 1; ++i) {
            ...
            res[i * 3 + 0] = F_prev[0] + F_next[0] + m * g_vec[0];
            res[i * 3 + 1] = F_prev[1] + F_next[1] + m * g_vec[1];
            res[i * 3 + 2] = F_prev[2] + F_next[2] + m * g_vec[2];
        ...
        if (norm_dx < TOL) {
            printf("\n✅ 3D Newton CONVERGED in %d steps.\n", iter);
            break;
        }
    }
    ...
    double* g_vec, double* out_positions)
{
    ...
    double *s_2d_n = malloc(n * sizeof(double));
    double *s_nl_n = malloc(n * sizeof(double));
    double *dx = malloc(dof * sizeof(double));

    if (!x || !s0_init || !s0_init_scaled || !s0_post_init || !s0_post_relax || !s_2d_n || !dx || !x_relaxed)
        return -1;
    ...
    // --- 2D Newton solver in rope plane ---

    int status = newton_solver_2d(P1, P2, g_vec, x_relaxed, n, s0_post_relax, c, m, out_positions);
    if (status != 0) {
        printf("❌ Newton solve in projected frame failed\n");
        free(x); free(s0_init); free(s0_init_scaled); free(s0_post_init);
        free(s0_post_relax); free(s_2d_n); free(dx),free(s_nl_n);
        return -1;
    }
    // Compute 2D Newton segment lengths
    double s_2d_n_sum = 0;
    for (int i = 0; i < n; ++i) {
        double xi[3], xi1[3];
        if (i == 0) {
            for (int j = 0; j < 3; ++j) {
                xi[j]  = P1[j];
                xi1[j] = out_positions[j];
            }
        } else if (i == n - 1) {
            for (int j = 0; j < 3; ++j) {
                xi[j]  = out_positions[(n - 2) * 3 + j];
                xi1[j] = P2[j];
            }
        } else {
            for (int j = 0; j < 3; ++j) {
                xi[j]  = out_positions[(i - 1) * 3 + j];
                xi1[j] = out_positions[i * 3 + j];
            }
        }
        double dx[3] = {
            xi1[0] - xi[0], xi1[1] - xi[1], xi1[2] - xi[2]
        };
        s_2d_n[i] = sqrt(dx[0]*dx[0] + dx[1]*dx[1] + dx[2]*dx[2]);
        s_2d_n_sum += s_2d_n[i];
    }
    printf("\nPositions after 2D Newton :\n");
    for (int i = 0; i < n - 1; ++i) {
        printf("s0_post_2D_Newton %d : %f    ", i, s_2d_n[i] );
        printf("Node %d: [%f, %f, %f]\n", i + 1,
            out_positions[i*3+0] , out_positions[i*3+1] , out_positions[i*3+2] );
    }
    printf("s0_post_2D_Newton %d : %f    \n",n-1,s0_post_relax[n-1]);
    printf("2D Newton Length = %f, Delta Length = %f]\n", s_2d_n_sum, s_2d_n_sum - L0);

    // --- 3D Newton solver ---
    double *x_3d = malloc(dof * sizeof(double));
    memcpy(x_3d, x_relaxed, sizeof(double) * dof);  // start from relaxed

    status = analytic_newton_solver_3d(x_3d, P1, P2, n, s0_post_relax, c, m, g_vec);
    if (status != 0) {
        printf("❌ 3D Newton solver failed\n");
        free(x_3d);
        return -1;
    }
    ...
    double s_3d_sum = 0;
    double *s3d = malloc(n * sizeof(double));
    for (int i = 0; i < n; ++i) {
        double xi[3], xi1[3];
        if (i == 0) {
            memcpy(xi, P1, 3 * sizeof(double));
            memcpy(xi1, &x_3d[0], 3 * sizeof(double));
        } else if (i == n - 1) {
            memcpy(xi, &x_3d[3 * (n - 2)], 3 * sizeof(double));
            memcpy(xi1, P2, 3 * sizeof(double));
        } else {
            memcpy(xi, &x_3d[3 * (i - 1)], 3 * sizeof(double));
            memcpy(xi1, &x_3d[3 * i], 3 * sizeof(double));
        }
        double dx[3] = {
            xi1[0] - xi[0], xi1[1] - xi[1], xi1[2] - xi[2]
        };
        s3d[i] = sqrt(dx[0]*dx[0] + dx[1]*dx[1] + dx[2]*dx[2]);
        s_3d_sum += s3d[i];
    }

    printf("\nPositions after 3D Newton :\n");
    for (int i = 0; i < n - 1; ++i) {
        printf("s0_post_3D_Newton %d : %f    ", i, s3d[i] );
        printf("Node %d: [%f, %f, %f]\n", i + 1,
            x_3d[i*3+0], x_3d[i*3+1], x_3d[i*3+2] );
    }
    printf("s0_post_3D_Newton %d : %f    \n", n - 1, s3d[n - 1]);
    printf("3D Newton Length = %f, Delta Length = %f]\n", s_3d_sum, s_3d_sum - L0);
    ...
    free(s0_post_init); free(s0_post_relax); free(s_2d_n);
    free(dx); free(s_nl_n);
    free(x_3d);
    free(s3d);
    return 0;
