    ...
    double dt_given, int max_steps)
{
    ...
    // --- Gravity component orthogonal to rope direction ---
    ...
    // --- Initialize straight line shape + sag ---
    double T0 = c * s0;
    ...
    double dt = fmin(dt_given, 0.05 / omega); // more conservative dt
    double damping = 2.5 * sqrt(c / m);
    double stop_threshold = 1e-4;
    double k_end_correction = 2.0 * c; // how hard we pull nodes back
