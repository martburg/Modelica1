    ...
    double dt_given, int max_steps)
{
    ...
    double sag_depth = 0.05 * L;


if (angle_deg < 10.0 || angle_deg > 170.0) {
    printf("Near-vertical case (%.2f°): initializing cubic parabola in rope–gravity plane\n", angle_deg);

    // e1: rope direction
    double e1[3] = { e_rope[0], e_rope[1], e_rope[2] };

    // Project gravity onto plane perpendicular to rope to get sag direction
    double dot = e1[0]*e_g[0] + e1[1]*e_g[1] + e1[2]*e_g[2];
    double g_proj[3] = {
        e_g[0] - dot * e1[0],
        e_g[1] - dot * e1[1],
        e_g[2] - dot * e1[2]
    ...
    if (g_proj_norm > 1e-12) {
        ...
        // Gravity perfectly aligned with rope
        e2[0] = e2[1] = e2[2] = 0.0;
    }

    // Now build 2D cubic parabola in (e1, e2) plane
    for (int i = 0; i < n-1; ++i) {
        double t = (double)(i + 1) / (double)n;

        double u = t * L;
        double v = -4.0 * sag_depth * t * (1.0 - t);

        // x_world = P1 + u * e1 + v * e2
        for (int j = 0; j < 3; ++j)
            x[i*3 + j] = P1[j] + u * e1[j] + v * e2[j];
    }
}
